...
read_mtx=function(dir,type="raw",min_umis=100){
  if (!is.na(type)){
...
read_mtx_and_split_by_10x_batch=function(path,libname,type="raw",noise_interval=c(40,100),cell_interval=c(201,20000)){
  l=list()
  min_umis=min(c(noise_interval,cell_interval))
  umitab=read_mtx(paste(path,libname,sep="/"),type,min_umis)
  
  ########
  # A.L. fix 3/28/19
  # edit colnames of umitab so that barcodes match exactly with barcodes in adt matrices
  colnames(umitab)=gsub("-1","",colnames(umitab))
  ########
...
read_mtx_and_split_by_hto=function(path,libname,type="raw",noise_interval=c(40,100),cell_interval=c(201,20000),hto_UMI_ratio_thresh=5,min_umi_maxhto=20,hto_to_sample=NULL){
  ...
  u=read_mtx(paste(path,"/",libname,sep=""),type = type,min_umis = min_umi)
...
read_and_compile_umitabs=function(sample_IDs,input_paths,output_path,filtered_or_raw="raw",prefix="",noise_interval=c(100,25000),cell_interval=c(200,25000),ds_numis=c(200,500,1000,2000),sample_annots_path=NULL,cite_seq_params=NULL,rename_cell_ids=F,only_load=T,output_qc_path=NULL){
  ...
  if (length(input_paths)==1){
    if (!"path"%in%colnames(annots_tab)){
  ...
  for (i in 1:length(amp_batches)){
    ...
    if (!cite_seq[i]){
      ...
      l=read_mtx_and_split_by_10x_batch(path = input_paths[i],libname = amp_batches[i],type = filtered_or_raw,noise_interval =noise_interval,cell_interval = cell_interval) 
      ...
      l=read_mtx_and_split_by_hto(input_paths[i],amp_batches[i],hto_UMI_ratio_thresh=cite_seq_params$hto_UMI_ratio_thresh,noise_interval=noise_interval,cell_interval=cell_interval,min_umi_maxhto=cite_seq_params$min_umi_maxhto,hto_to_sample=hto_to_sample)
      if (!is.null(output_qc_path)){
        dev.off()
      }
...
read_multiple_mtx=function(paths,libnames,sample_annots_path=NULL,noise_interval=c(40,100),cell_interval=c(201,20000),rename_cell_ids=T,  type="raw",cite_seq_params=NULL){
  if (is.null(cite_seq_params)){
    cite_seq_params=list()
    cite_seq_params$hto_UMI_ratio_thresh=5
    cite_seq_params$min_umi_maxhto=20
  ...
  for (i in 1:length(libnames)){
    ...
    if (file.exists(paste(paths[i],libnames[i],"hto.txt",sep="/"))){
      l_umitab_i=read_mtx_and_split_by_hto(paths[i],libnames[i],hto_UMI_ratio_thresh=cite_seq_params$hto_UMI_ratio_thresh,noise_interval=noise_interval,cell_interval=cell_interval,min_umi_maxhto=cite_seq_params$min_umi_maxhto,htos_to_load =strsplit(annots_tab[i,]$HTO,",")[[1]])
    }
    else{
      l_umitab_i=read_mtx_and_split_by_10x_batch(paths[i],libnames[i],type,noise_interval=noise_interval,cell_interval=cell_interval)
    }
