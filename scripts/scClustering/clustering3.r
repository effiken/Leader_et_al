...
update_alpha=function(umitab,models,noise_models,cell_to_batch,reg,max_noise_fraction=1e-10,cell_to_cluster=NULL,max_n_cores=1){
...
get_seed=function(k,train_umitab,ds,cell_to_batch,noise_models,high_var_genes,params=NULL,model_name="",km_i=NA,trace=F){
  
  if (params$init_method=="TGL_kmeans"){
    library(tglkmeans)
  }
  else if (params$init_method=="kmeans_rcpp"){
    library(ClusterR)
  }
  else if (params$init_method=="skmeans"){
    library(skmeans)
  }
  ...
  if (params$redownsample){
   ...
   # png(paste("saved_clustering/",model_name,"/tmp/varmean_output_seed_",km_i,".png",sep=""))
  ...
  if (params$init_method=="kmeans"){
    # traditional kmeans 
    m_to_km=log2((params$km_reg+ds[high_var_genes,])/(params$km_reg+rowMeans(ds[high_var_genes,])))
    iter.max=20
    km=kmeans(m_to_km,k,iter.max=10,nstart = 1)
    m_to_km=t(ds[high_var_genes,])
    cell_to_cluster=km$cluster
  }
  # kmeans_rcpp kmeans++
  else if (params$init_method=="kmeans_rcpp"){
    m_to_km=as.matrix(log2(t((params$km_reg+ds[high_var_genes,]))))
    km1=KMeans_rcpp(m_to_km, clusters = k, num_init = 1, max_iters = 100, initializer = 'kmeans++', verbose = T,seed =round(runif(1)*1e6) )
    cell_to_cluster=as.character(km1$cluster)
    #pdc_genes=strsplit(read.delim("~/GoogleDrive/work/shared/clustering_data/clustering_data_IBD/gene_sets.txt",stringsAsFactors = F,row.names=1)["pDC_genes",],",")[[1]]
    ordg=get_order(seriate(as.dist(1-(cor((km1$centroids)))),method = "OLO"))
    ordc=get_order(seriate(as.dist(1-(cor(t(km1$centroids)))),method = "OLO"))
    image(km1$centroids[ordc,ordg],col=greenred(100))
    #rowSums(km1$centroids[,match(pdc_genes,colnames(m_to_km))],na.rm=T)[ordc]
   
    names(cell_to_cluster)=rownames(m_to_km)
    rm(m_to_km)
    rm(km1)
  }
  #TGLKmeans
  else if (params$init_method=="TGL_kmeans"){
    ...
    cell_to_cluster=as.character(km1$cluster)
    names(cell_to_cluster)=rownames(df_to_km)
  #  browser()
  # ordg=get_order(seriate(as.dist(1-(cor((km1$centers)))),method = "OLO"))
  #  ordc=get_order(seriate(as.dist(1-(cor(t(km1$centers)))),method = "OLO"))
  #  image(km1$centers[ordc,ordg],col=greenred(100))
  #  rowSums(km1$centers[,match(pdc_genes,colnames(km1$centers))],na.rm=T)[ordc]
    ...
    ###skmeans
    ads=Matrix(log2(1+ds[high_var_genes,]))
    km=skmeans(t(ads),k,control=list(start=list('p')),method="pclust")
    cell_to_cluster=km$cluster
    names(cell_to_cluster)=rownames(ads)
    rm(ads)
  }
 ...
 # alpha_noise_test=update_alpha(umitab = init_test_umitab[params$clustering_genes,],models = models,noise_models = noise_models,cell_to_batch = cell_to_batch[colnames(init_test_umitab)],reg=params$reg)
 ...
 # train_total_ll_per_batch=sapply(split(train_bcl$ll,cell_to_batch[rownames(train_bcl$ll)]),mean)
  #test_total_ll_per_batch=sapply(split(init_test_bcl$ll,cell_to_batch[rownames(init_test_bcl$ll)]),mean)
  
#  test_tab=table(cell_to_cluster_test)
#  train_tab=table(cell_to_cluster)
  
 # pdf(paste("saved_clustering/",model_name,"/tmp/cluster_freq_seed_",km_i,".pdf",sep=""))
#  plot(as.vector(train_tab/sum(train_tab)),as.vector(test_tab/sum(test_tab)),panel.first={abline(0,1,lty=2);grid(lty=1)},xlab="Training set",ylab="Test set",main="Cluster Frequency")
 # dev.off()

  #mat_ll_per_batch=round(cbind(ll_train=train_total_ll_per_batch,ll_seed_test=test_total_ll_per_batch),digits=3)
...
get_seed_launcher=function(train_umitab,ds,high_var_genes,cell_to_batch,noise_models,k,data_l_path=NULL,params,model_name,running_mode,parallel_params=NULL,trace=F){
  
  get_seed_wrapper=function(x){
    res=get_seed(k,train_umitab,ds,cell_to_batch,noise_models,high_var_genes,params=params,model_name=model_name,km_i=x,trace=trace);
  ...
  if (running_mode=="local"){
    ...
    save(k,train_umitab,ds,cell_to_batch,noise_models,high_var_genes,params,model_name,file = tmp_seed_rd_filename)
...
init_clusters_kmeans=function(train_umitab,k,cell_to_batch,noise_models,params,model_name,running_mode,parallel_params=NULL,data_l_path,trace=F){
  ...
  seeding_genes=rownames(train_umitab)[(!(rownames(train_umitab)%in%params$genes_excluded_from_seeding))&(!(rownames(train_umitab)%in%params$genes_excluded))]
  ...
  l=get_seed_launcher(train_umitab=train_umitab,
    ds=ds,
