...
if(!dir.exists("output/modules")){
  dir.create("output/modules")
}
modules <- parse_modules_leader(ldm=lung_ldm,cormat = cmat,ds_version = "2000", modules_version = "Mono_mac",
                                      tables_path = "output/modules/",nmods = 50,min_mod_cor = 0.1)

modules <- read.table("output/modules/Mono_mac_modules.txt",sep="\t",stringsAsFactors=F)$V1
